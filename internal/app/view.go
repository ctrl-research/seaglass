...
type streamer interface {
	Stream(ctx context.Context, res k8s.Resource, namespace, fieldSelector, labelSelector string, fullObjects bool) <-chan k8s.Update
}
...
func (s clientStreamer) Stream(ctx context.Context, res k8s.Resource, ns, fieldSelector, labelSelector string, fullObjects bool) <-chan k8s.Update {
	return s.c.Stream(ctx, res.GVR, ns, fieldSelector, labelSelector, fullObjects)
}
